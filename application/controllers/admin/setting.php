...
class Setting extends MY_admin
{
	...
	function technical()
	{
		...
		$this->template['db_pass'] = 	$db[$active_group]['password'];
	...
	*/
	...
	//20140415 zoearth
	function save_technical_settings()
	{
	    //20140415 zoearth email 部分
	    ...
	    if ($data['protocol'] == '' )
	    {
	        $this->error(lang('ionize_message_smtp_not_saved'),NULL,'setting/technical');
	    }
	    else
	    {
	        $this->load->helper('file');
	        ...
	        {
	            $this->error(lang('ionize_message_error_writing_email_file'),NULL,'setting/technical');
	        }
	    }
	    
	    //20140415 zoearth DB 設定
	    $db_config = array(
	            'hostname'    =>  $_POST['db_host'],
	            'username'    =>  $_POST['db_user'],
	            'password'    =>  $_POST['db_pass'],
	    ...
	    // If data are missing : Redirect || error
	    if ($db_config['hostname'] == '' || $db_config['dbdriver'] == '' || $db_config['database'] == '' || $db_config['username'] == '')
	    {
	        $this->error(lang('ionize_message_database_not_saved'),NULL,'setting/technical');
	    }
	    $dsn = $db_config['dbdriver'].'://'.$db_config['username'].':'.$db_config['password'].'@'.$db_config['hostname'].'/'.$db_config['database'];
	    $db = DB($dsn, TRUE, TRUE);
	    $db->db_connect();
	    if ( ! $db->db_select()  )
	    {
	        $this->error(lang('ionize_message_database_connection_error'),NULL,'setting/technical');
	    }
	    else
	    {
	        $this->load->helper('file');
	        ...
	        if ( ! write_file(APPPATH.'config/database.php', $conf))
	        {
	            $this->error(lang('ionize_message_error_writing_database_file'),NULL,'setting/technical');
	        }
	        else
	        {
	            $this->success(lang('ionize_message_database_saved'),NULL,'setting/technical');
	        }
	    }
	}
